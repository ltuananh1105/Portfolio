...
const chapters = ["Overview", "Context", "Process", "Requirements", "Traceability", "System Analysis", "Data Model", "System Flow", "Validation", "Product", "Change Impact", "Reflection"];
...
function EvidenceLink({ href, children }) {
  return href ? <a className="lu-button" href={href} target="_blank" rel="noreferrer">{children}<span aria-hidden="true"> ↗</span></a>
    : <span className="lu-unavailable"><button className="lu-button" disabled>{children}</button><small>Link not yet provided</small></span>;
}
...
function Asset({ asset, onExpand, product = false, hero = false }) {
  return <figure className={`lu-asset ${product ? "lu-product-asset" : "lu-diagram"} ${hero ? "lu-cover-asset" : ""}`}>
    ...
    {asset.src ? <button className="lu-image-button" onClick={() => onExpand(asset)} aria-label={`Expand ${asset.title}`}>
    ...
    </button> : <div className="lu-placeholder"><span className="lu-placeholder-symbol" aria-hidden="true">{product ? "▧" : "◇"}</span><p className="lu-eyebrow">{product ? "Screenshot placeholder" : "Diagram placeholder"}</p><strong>{asset.title}</strong><p>{product ? "A real application capture belongs here." : "Original diagram awaiting a web-compatible export."}</p>{asset.file && <code>{asset.file}</code>}</div>}
...
function LearnUpCaseStudy() {
  ...
  return <main className="learnup-case-study" ref={pageRef}>
    ...
    <div className="lu-shell">
      ...
      <nav className="lu-nav" aria-label="LearnUp chapters"><span className="lu-nav-progress">{number(active)} <span>/ 12</span></span>
      ...
      <Section index={1} title={<>One platform. Three core roles.<br /><em>One connected course lifecycle.</em></>}>
      ...
      <Section index={2} label="Process Analysis" title={<>How does a course<br /><em>become available to students?</em></>} intro="Creation is only the beginning. Ownership, submission validation and administrative review govern publication.">
        <Asset asset={diagrams.courseReview} onExpand={expand} />
        ...
        <div className="lu-secondary-process"><div><p className="lu-eyebrow">The student side</p><h3>From discovery<br />to course access.</h3><p>Enrollment connects a published course to a learner. The academic implementation includes simulated payment and duplicate enrollment prevention.</p><code>FR-02 · UC-05 · BR-02</code></div><Asset asset={diagrams.enrollment} onExpand={expand} /></div>
      </Section>
      <Section index={3} title={<>Turning system needs<br /><em>into structured requirements.</em></>}>
        ...
        <div className="lu-requirement-path">{[["BRQ-02", "Discovery & Enrollment"], ["FR-02", "Student Enrollment"], ["UC-05", "Enroll in Course"], ["US-05", "Student Enrollment User Story"], ["BR-02", "Prevent Duplicate Enrollment"], ["AC-07 / AC-08 / AC-09", "Acceptance criteria"]].map(([id, text]) => <div key={id}><code>{id}</code><span>{text}</span></div>)}</div>
      </Section>
      <Section index={4} className="lu-signature" label="Traceability" title={<>From business objective<br />to <em>validated system behavior.</em></>} intro="One requirement. Traced from business objective to validated system behavior.">
        <div className="lu-trace-intro"><span className="lu-pill">Enrollment / End-to-end evidence</span><span className="lu-footnote">Read the chain from top to bottom</span></div>
        <ol className="lu-trace">{trace.map(([label, id, text], index) => <li key={id} className={index === trace.length - 1 ? "lu-trace-outcome" : ""}><span className="lu-trace-marker" aria-hidden="true">{number(index)}</span><span className="lu-trace-label">{label}</span><div><code>{id}</code><p>{text}</p></div></li>)}</ol>
        <div className="lu-trace-end"><p>Traceability connects the reason for a feature to the evidence that its rules hold.</p><EvidenceLink href={evidenceLinks.rtm}>View Full RTM</EvidenceLink></div>
      </Section>
      <Section index={5} title={<>Connecting requirements<br /><em>with system behavior.</em></>}>
      ...
      <Section index={6} title={<>Requirements don’t stop<br /><em>at the interface.</em></>} intro="The data model represents course structure, enrollment, learning progress and assessment results.">
      ...
      <Section index={7} title={<>From user action<br /><em>to system response.</em></>}>
      ...
      <Section index={8} title={<>Did the implemented system<br /><em>match the requirements?</em></>}>
      ...
      <Section index={9} label="Implementation" title={<>From analysis<br /><em>to a working system.</em></>} intro="The academic application connects these requirements to teacher, admin and student workflows. Real UI captures are pending.">
        <div className="lu-products">{screenshots.map((asset) => <article key={asset.id}><Asset asset={asset} product onExpand={expand} /><div className="lu-product-caption"><code>{asset.id}</code><p>{asset.caption}</p></div></article>)}</div>
      </Section>
      <Section index={10} title={<>What happens<br /><em>when a requirement changes?</em></>}>
      ...
      <Section index={11} title={<>What this project<br /><em>taught me.</em></>}>
