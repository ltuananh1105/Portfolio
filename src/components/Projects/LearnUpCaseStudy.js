...
const chapterKeys = ["overview", "process", "requirements", "system-design", "validation", "reflection"];
...
function LearnUpCaseStudy() {
  ...
  const activeChapter = chapterKeys.includes(requestedChapter) ? requestedChapter : "overview";
  ...
  function changeChapter(index) {
    ...
    navigate({ pathname: location.pathname, search: location.search, hash: chapterKeys[index] });
  }
  useEffect(() => {
  ...
  return <main className="learnup-case-study">
    <Particle />
    <div className="lu-shell">
      <nav className="lu-nav" aria-label="LearnUp chapters"><span className="lu-nav-progress">{number(active)} <span>/ {chapters.length}</span></span>
        <div className="lu-nav-links">{chapters.map((chapter, index) => <button type="button" key={chapter} onClick={() => changeChapter(index)} aria-current={active === index ? "step" : undefined} aria-controls="lu-chapter-stage"><span>{number(index)}</span> {chapter}</button>)}</div>
        <label className="lu-mobile-nav">Chapter<select aria-label="Jump to chapter" value={active} onChange={(event) => changeChapter(Number(event.target.value))}>{chapters.map((chapter, index) => <option key={chapter} value={index}>{number(index)} / {chapter}</option>)}</select></label>
      </nav>
      <div id="lu-chapter-stage" className="lu-chapter-stage" key={activeChapter} ref={chapterRef} tabIndex={-1} aria-label={`${chapters[active]} chapter`} >
      {activeChapter === "overview" && <section className="lu-hero" id={chapterId(0)} aria-labelledby="lu-hero-title">
      ...
      {activeChapter === "process" && <Section index={1} label="Process Analysis" title={<>From course creation<br /><em>to student enrollment.</em></>} intro="Creation is only the beginning. Ownership, submission validation and administrative review govern publication.">
      ...
      {activeChapter === "requirements" && <Section index={2} label="Requirements & Traceability" title={<>Turning system needs<br />into <em>validated behavior.</em></>}>
        ...
        <div className="lu-signature">
          ...
          <ol className="lu-trace-groups">{["Why", "What", "Rule & expectation", "Prove"].map((group, groupIndex) => <li className="lu-trace-group" key={group}>
            ...
            <ol>{trace.slice(groupIndex * 2, groupIndex * 2 + 2).map(([label, id, text], stepIndex) => <li key={id} className={groupIndex === 3 && stepIndex === 1 ? "lu-trace-result" : ""}>
              <span className="lu-trace-label">{label}</span><p>{text}</p><code>{id}</code>
            </li>)}</ol>
          </li>)}</ol>
      ...
      {activeChapter === "system-design" && <Section index={3} label="System Design" title={<>From requirements<br /><em>to system structure.</em></>}>
        ...
        <div className="lu-mappings">{mappings.map(([id, title, api, logic, data]) => <article key={id}><div><h4>{title}</h4><code>{id}</code></div><div><span>API / capability</span><code>{api}</code></div><div><span>Business logic</span><p>{logic}</p></div><div><span>{id === "FR-08" ? "Response" : "Data impact"}</span><p>{data}</p></div></article>)}</div>
      ...
      {activeChapter === "validation" && <Section index={4} label="Validation & Working Product" title={<>From requirement<br /><em>to working behavior.</em></>}>
      ...
      {activeChapter === "reflection" && <Section index={5} title={<>What this project<br /><em>taught me.</em></>}>
      ...
      <nav className="lu-chapter-controls" aria-label="Previous and next chapter">
        {active > 0 ? <button type="button" onClick={() => changeChapter(active - 1)}>← Previous: {chapters[active - 1]}</button> : <span />}
        <span className="lu-chapter-count">{number(active)} / {chapters.length}</span>
        {active < chapters.length - 1 ? <button type="button" onClick={() => changeChapter(active + 1)}>Next: {chapters[active + 1]} →</button> : <span />}
      </nav>
      </div>
    </div>
